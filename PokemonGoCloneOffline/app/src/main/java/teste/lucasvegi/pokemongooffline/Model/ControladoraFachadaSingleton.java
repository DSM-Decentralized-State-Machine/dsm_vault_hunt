...
import teste.lucasvegi.pokemongooffline.Util.OSMNearbySearch;
...
public final class ControladoraFachadaSingleton {
    ...
    public List<Pokestop> getPokestops(double latitude, double longitude){
        ...
        // Use OpenStreetMap for nearby search instead of Google Places API
        OSMNearbySearch.OSMSearchResponse response = OSMNearbySearch.run(latitude, longitude);
        OSMNearbySearch.OSMPlace[] places = response.results;
        
        if(places != null && places.length > 0) {
            for (int i = 0; i < places.length && i < places.length / 2; i++) {
                double lat = places[i].lat;
                double lng = places[i].lon;

                Pokestop pokestop = new Pokestop("osm_" + places[i].id, places[i].name);
                pokestop.setlat(lat);
                pokestop.setlong(lng);
                if (places[i].type != null && !places[i].type.isEmpty())
                    pokestop.setDescri(places[i].type);

                //TODO : setar imagem do pokestop dentro da classe do pokestop
                // OpenStreetMap doesn't provide photos API, so we skip this part
    ...
    // This method is deprecated as we've moved from Google Places to OpenStreetMap
    // OpenStreetMap doesn't provide a photo API, so we'll use default images instead
    private void getPlaceImage(Pokestop pokestop) {
        // Method kept for compatibility but no longer needed with OpenStreetMap
        // A default image will be used for Pokestops
    }
